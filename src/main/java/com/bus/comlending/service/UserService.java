...
import com.bus.comlending.service.util.RandomUtil;
...
public class UserService {
    ...
    public Optional<User> confirmRegistration(String token) {
        log.debug("Confirming user for token id {}", token);
        Instant curtime = Instant.now();
        LocalDateTime curLocalDateTime = LocalDateTime.now();
        Optional<User> filteruser = null;
        try{
            filteruser = userRepository.findOneBytokennumber(token)
                .map(user -> {
                    // activate given user for the registration key.
                    Instant cdate = user.getCreatedDate();
                    LocalDateTime localCreatedDate = LocalDateTime.ofInstant(cdate,ZoneOffset.systemDefault());

                    long timediff = localCreatedDate.until(curLocalDateTime, ChronoUnit.MINUTES);
            ...
            //Object obj = filteruser.getClass();
